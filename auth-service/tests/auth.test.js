...
describe("🔐 Auth Service M2 Tests", () => {
  ...
  test("✅ Logout fonctionne", async () => {
    const res = await request(app).post("/auth/logout");
    expect([200, 500]).toContain(res.statusCode);
  });

  test("✅ Metrics endpoint répond", async () => {
    const res = await request(app).get("/metrics").expect(200);

    expect(res.headers["content-type"]).toContain("text/plain");
  });

  test("✅ Vitals endpoint répond", async () => {
    const res = await request(app).get("/vitals").expect(200);

    expect(res.body.service).toBe("auth-service");
  });

  test("✅ Rate limiting fonctionne", async () => {
    const res = await request(app).get("/health");
    expect([200, 503, 429]).toContain(res.statusCode);
  });

  test("✅ Gestion erreur OAuth", async () => {
