...
export default function CreateRoadTrip() {
  ...
  // Dans votre composant CreateRoadTrip, modifiez la fonction handleSaveRoadtrip
  ...
  const handleSaveRoadtrip = useCallback(
    async (publish = false) => {
      ...
      try {
        ...
        const response = await AdminService.createRoadtrip(roadtripToSave);

        // 🔥 DEBUG : Voir la structure de réponse
        console.log("🔍 Réponse complète createRoadtrip:", response);
        ...
        if (response.trip && response.trip._id) {
          roadtripId = response.trip._id;
        } else if (response.trip && response.trip.id) {
          roadtripId = response.trip.id;
        } else if (response.data && response.data._id) {
          roadtripId = response.data._id;
        } else if (response.data && response.data.id) {
          roadtripId = response.data.id;
        } else if (response._id) {
          roadtripId = response._id;
        } else if (response.id) {
          roadtripId = response.id;
        }

        console.log("🔍 ID extrait:", roadtripId);

        // Valider que l'ID est un ObjectId MongoDB valide
        if (!roadtripId || !/^[0-9a-fA-F]{24}$/.test(roadtripId)) {
          console.error("❌ ID de roadtrip invalide:", roadtripId);
          console.error("❌ Structure de réponse:", response);

          showAlert(
            `Roadtrip ${
              publish ? "publié" : "sauvegardé"
            } avec succès ! Redirection vers la liste...`,
            "success"
          );

          // Redirection vers admin au lieu de la page du roadtrip
          setTimeout(() => {
            router.push("/admin?tab=roadtrips");
          }, 1500);
          return;
        }
        ...
        // Redirection vers la page du roadtrip avec l'ID correct
        setTimeout(() => {
