...
export const ChatInput = memo(
  forwardRef<HTMLInputElement, ChatInputProps>(
    ({ value, onChange, onSubmit, disabled = false, placeholder, isMobile = false }, ref) => {
      const val = value ?? "";
      const canSubmit = !disabled && val.trim().length > 0;
        ...
        <footer className="border-t border-stone-200 bg-white/95 backdrop-blur-sm p-4 md:p-6 shadow-lg">
          ...
          <div className="text-center mt-3">
            <p className="text-xs text-stone-400">
              {disabled ? "Traitement en cours..." : "Tapez votre question et appuyez sur Entrée"}
            </p>
          </div>
        </footer>
      );
