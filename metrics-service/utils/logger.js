...
// 📁 Structure des logs adaptée à ROADTRIP - CORRECTION POUR RENDER
const createLogsPaths = (serviceName) => {
  // Détecter l'environnement cloud (Render, Heroku, etc.)
  ...
  const isCloudEnvironment = isRender || process.env.HEROKU || process.env.AWS_LAMBDA_FUNCTION_NAME;
  
  let baseLogsDir;
  let serviceLogsDir;
  
  // CORRECTION: Gestion intelligente des environnements
  if (isCloudEnvironment || isRender) {
    // Sur Render et autres plateformes cloud : utiliser /tmp obligatoirement
    baseLogsDir = '/tmp/logs';
    serviceLogsDir = path.join(baseLogsDir, serviceName);
    console.log(`☁️  Environnement cloud détecté, utilisation de /tmp/logs`);
  } else if (isDocker) {
    // Dans Docker: utiliser le volume monté ou /app/logs
  ...
  // CORRECTION: Fonction helper pour créer les dossiers avec fallback automatique
  const createDirSafe = (dir) => {
    try {
  ...
  // Essayer de créer le dossier principal
  if (!createDirSafe(baseLogsDir)) {
    // Si échec, fallback automatique vers /tmp
    console.warn(`⚠️  Fallback vers /tmp/logs pour ${serviceName}`);
    baseLogsDir = '/tmp/logs';
    serviceLogsDir = path.join(baseLogsDir, serviceName);
    
    // Créer le fallback (doit fonctionner sur tous les systèmes)
    createDirSafe(baseLogsDir);
  }
  
  // Créer le dossier du service
  createDirSafe(serviceLogsDir);
...
// CORRECTION: Transports fichiers avec gestion d'erreur améliorée
  ...
  try {
    loggerTransports.push(
      ...
      new transports.File({
        ...
        maxFiles: 5,
        tailable: true,
        handleExceptions: false // Éviter les boucles infinies
      }),
    ...
    console.log(`✅ Logs fichiers activés dans ${logsPaths.serviceDir}`);
  } catch (error) {
    console.warn(`⚠️  Impossible d'activer les logs fichiers: ${error.message}`);
    console.log(`📝 Utilisation des logs console uniquement`);
  }
}
